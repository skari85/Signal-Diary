...
export function useSignalDiary() {
  ...
  const loadLogs = useCallback(() => {
    try {
      ...
      if (savedLogs) {
        ...
        const uniqueLocations = [...new Set(locations)].slice(0, 5) as string[]
  ...
  // Add new log entry
  const addLogEntry = useCallback(async (type: LogEntry["type"], location: string) => {
    ...
    try {
      ...
      if (finalLocation !== "Not specified") {
        const newRecentLocations = [
          finalLocation, 
          ...recentLocations.filter((loc) => loc !== finalLocation)
        ].slice(0, 5)
  ...
  const getLogsByLocation = useCallback((location: string) => {
